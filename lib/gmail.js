...
/**
 * Redact access/refresh tokens and bearer credentials from a log string.
 * @param {string} text
 * @returns {string}
 */
function redactSecretsInString(text) {
...
/**
 * Deep-sanitize a Google error payload for logging (tokens stripped; depth-capped).
 * @param {unknown} value
 * @param {number} [depth]
 * @returns {unknown}
 */
function sanitizeGoogleErrorValue(value, depth = 0) {
  if (depth > 6) return '[truncated]';
  if (value == null) return value;
  if (typeof value === 'string') return redactSecretsInString(value).slice(0, 2000);
  if (typeof value === 'number' || typeof value === 'boolean') return value;
  if (Array.isArray(value)) {
    return value.slice(0, 20).map((item) => sanitizeGoogleErrorValue(item, depth + 1));
  }
  if (typeof value === 'object') {
    /** @type {Record<string, unknown>} */
    const out = {};
    for (const [key, child] of Object.entries(value)) {
      if (/authorization|access_token|refresh_token|id_token|client_secret|password/i.test(key)) {
        // Omit auth material entirely — do not even leave the header name in logs.
        continue;
      }
      out[key] = sanitizeGoogleErrorValue(child, depth + 1);
    }
    return out;
  }
...
/**
 * Prefer Google's semantic status/reason (e.g. PERMISSION_DENIED) over bare HTTP codes.
 * @param {unknown} err
 */
function getSafeErrorCode(err) {
  ...
  if (nestedError && typeof nestedError === 'object') {
    const nested = /** @type {Record<string, unknown>} */ (nestedError);
    const statusName = safeCodeString(nested.status);
    if (statusName) return statusName;
    const errors = Array.isArray(nested.errors) ? nested.errors : [];
    if (errors[0] && typeof errors[0] === 'object') {
      const reason = safeCodeString(/** @type {Record<string, unknown>} */ (errors[0]).reason);
      if (reason) return reason;
    }
  } else if (typeof nestedError === 'string') {
    const asCode = safeCodeString(nestedError);
    if (asCode) return asCode;
  }

  return safeCodeString(anyErr.code) || null;
}

/**
 * Extract Google API failure details for ops debugging.
 * Never includes access/refresh tokens or Authorization headers.
 * @param {unknown} err
 * @returns {{
 *   googleErrorMessage: string | null,
 *   googleErrors: unknown,
 *   googleResponseData: unknown,
 *   requestUrl: string | null,
 * }}
 */
function extractGoogleApiErrorDetails(err) {
  const empty = {
  ...
  const response =
    anyErr.response && typeof anyErr.response === 'object'
      ...
      : null;
  const config =
    (anyErr.config && typeof anyErr.config === 'object'
      ? /** @type {Record<string, unknown>} */ (anyErr.config)
      : null) ||
    (response?.config && typeof response.config === 'object'
      ? /** @type {Record<string, unknown>} */ (response.config)
      : null);
  ...
  let googleErrors = null;

  if (nestedError && typeof nestedError === 'object') {
    const nested = /** @type {Record<string, unknown>} */ (nestedError);
    if (typeof nested.message === 'string') {
      googleErrorMessage = redactSecretsInString(nested.message).slice(0, 2000);
    }
    if (Array.isArray(nested.errors)) {
      googleErrors = sanitizeGoogleErrorValue(nested.errors);
    }
  } else if (typeof nestedError === 'string') {
    googleErrorMessage = redactSecretsInString(nestedError).slice(0, 2000);
  }

  // Only fall back to err.message for Google/Gaxios-style failures that include a response.
  // Bare local throws must not dump secrets into ops logs.
  if (!googleErrorMessage && response && typeof anyErr.message === 'string') {
    googleErrorMessage = redactSecretsInString(anyErr.message).slice(0, 2000);
  }

  if (!googleErrors && response && Array.isArray(anyErr.errors)) {
    googleErrors = sanitizeGoogleErrorValue(anyErr.errors);
  }

  let requestUrl = null;
  const rawUrl =
    (typeof config?.url === 'string' && config.url) ||
    (typeof anyErr.url === 'string' && anyErr.url) ||
    (response?.request &&
      typeof response.request === 'object' &&
      typeof /** @type {Record<string, unknown>} */ (response.request).responseURL === 'string' &&
      /** @type {Record<string, unknown>} */ (response.request).responseURL) ||
    null;
  if (typeof rawUrl === 'string' && rawUrl) {
    const base =
      config && typeof config.baseURL === 'string' && !/^https?:\/\//i.test(rawUrl)
        ? String(config.baseURL).replace(/\/$/, '')
        : '';
    const joined = base ? `${base}/${rawUrl.replace(/^\//, '')}` : rawUrl;
    try {
      const parsed = new URL(joined);
      for (const key of ['access_token', 'refresh_token', 'id_token', 'client_secret', 'code']) {
        if (parsed.searchParams.has(key)) parsed.searchParams.set(key, '[REDACTED]');
      }
      requestUrl = redactSecretsInString(parsed.toString()).slice(0, 500);
    } catch (_e) {
      requestUrl = redactSecretsInString(joined).slice(0, 500);
    }
  }
  ...
  return {
    googleErrorMessage,
    googleErrors,
    googleResponseData: responseData != null ? sanitizeGoogleErrorValue(responseData) : null,
    requestUrl,
  };
}
 ...
 * @template T
 * @param {() => Promise<T>} fn
 * @param {{ env?: Record<string, unknown>, maxRetries?: number, sleep?: (ms: number) => Promise<void>, baseDelayMs?: number, accountId?: string | null, operation?: string }} [opts]
 * @returns {Promise<T>}
 */
    ...
    try {
      ...
      if (attempt >= maxRetries || !isRetryableGoogleError(err)) {
        logGmailEvent({
          ...
          operation: opts.operation || 'gmail_api_request',
          err,
          retryCount: attempt,
        });
        throw err;
...
/**
 * Structured Gmail operational logging.
 * When `err` is provided, includes sanitized Google API failure details
 * (message, errors[], response.data, request URL) without tokens.
 * @param {object} fields
 */
function logGmailEvent(fields = {}) {
  const fromErr =
    fields.err !== undefined ? extractGoogleApiErrorDetails(fields.err) : {
      googleErrorMessage: fields.googleErrorMessage || null,
      googleErrors: fields.googleErrors || null,
      googleResponseData: fields.googleResponseData || null,
  ...
  const event = {
    ...
    errorCode:
      fields.errorCode ||
      (fields.err !== undefined ? getSafeErrorCode(fields.err) : null) ||
      null,
    httpStatus:
      fields.httpStatus ||
      (fields.err !== undefined ? getHttpStatus(fields.err) : null) ||
      null,
    ...
    googleErrorMessage: fromErr.googleErrorMessage,
    googleErrors: fromErr.googleErrors,
    googleResponseData: fromErr.googleResponseData,
    requestUrl: fromErr.requestUrl,
  };
...
async function getAuthorizedClient(accountId, env) {
  ...
  if (needsRefresh) {
    try {
      ...
      const refreshed = await withGoogleRetries(() => adapter.refreshAccessToken(oauth2Client), {
        env,
        accountId,
        operation: 'gmail_refresh_access_token',
      });
...
async function handleOAuthCallback(query, env) {
  ...
  let tokens;
  try {
    tokens = await withGoogleRetries(() => adapter.exchangeCode(oauth2Client, code), {
      env,
      operation: 'gmail_oauth_exchange_code',
    });
  } catch (_e) {
  ...
  let profile;
  try {
    profile = await withGoogleRetries(() => adapter.getProfile(oauth2Client), {
      env,
      operation: 'gmail_oauth_get_profile',
    });
  } catch (_e) {
 ...
 * @param {string} [accountId]
 */
async function listMessageIdsPaginated(adapter, oauth2Client, config, env, accountId) {
  const messageIds = [];
  const seen = new Set();
  let pageToken;
  let capped = false;
  ...
  do {
    ...
    const listed = await withGoogleRetries(
      ...
      { env, accountId, operation: 'gmail_list_messages' }
    );
 ...
 * @param {string} [accountId]
 */
async function listHistoryBounded(adapter, oauth2Client, startHistoryId, config, env, accountId) {
  ...
  do {
    ...
    const history = await withGoogleRetries(
      ...
      { env, accountId, operation: 'gmail_list_history' }
    );
...
async function syncGmailAccount(accountId, env) {
  ...
  try {
    ...
    try {
      ensureNotTimedOut();
      if (!priorCursor) {
        const listed = await listMessageIdsPaginated(adapter, oauth2Client, config, env, accountId);
        messageIds = listed.messageIds;
        capped = listed.capped;
        capReason = listed.capReason;
        const profile = await withGoogleRetries(() => adapter.getProfileHistoryId(oauth2Client), {
          env,
          accountId,
          operation: 'gmail_get_profile_history_id',
        });
        nextCursor = profile.historyId || null;
      } else {
        try {
          const history = await listHistoryBounded(
            adapter,
            oauth2Client,
            priorCursor,
            config,
    ...
    try {
      for (const messageId of messageIds) {
        ensureNotTimedOut();
        const raw = await withGoogleRetries(() => adapter.getMessage(oauth2Client, messageId), {
          env,
          accountId,
...
module.exports = {
  ...
  getHttpStatus,
  getSafeErrorCode,
  extractGoogleApiErrorDetails,
  redactSecretsInString,
  sanitizeGoogleErrorValue,
};
