...
/**
 * Money amount: never store as float. Accept string or non-negative integer,
 * normalize to a decimal string with up to 4 fractional digits.
 * Zero and negative amounts are rejected (no zero-value payment records).
 */
const amountSchema = z
  ...
  .transform((value, ctx) => {
    ...
    if (!/^(?:0|[1-9]\d*)(?:\.\d{1,4})?$/.test(trimmed)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'amount must be a positive decimal string with up to 4 fractional digits',
      });
      return z.NEVER;
    }
    // Reject zero (including 0, 0.0, 0.0000) — payments must have a positive amount.
    if (/^0+(?:\.0+)?$/.test(trimmed)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'amount must be greater than zero' });
      return z.NEVER;
    }
