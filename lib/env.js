...
const envSchema = z.object({
  ...
  // Email AI analysis (opt-in). When disabled, mock provider is used and AI keys are not required for analysis.
  AI_EMAIL_ANALYSIS_ENABLED: z.string().optional(),
  AI_PROVIDER: z.string().optional(),
  AI_MODEL: z.string().optional(),
  AI_API_KEY: z.string().optional(),
  // Staging-only admin tools (e.g. POST /admin/reset-analysis). Set true on Railway staging when NODE_ENV=production.
  STAGING_ADMIN_TOOLS: z.string().optional(),
...
const AI_EMAIL_ANALYSIS_PROVIDERS = ['mock', 'anthropic'];

/**
 * True when AI email analysis is explicitly enabled.
 * @param {NodeJS.ProcessEnv | Record<string, string | undefined>} source
 */
function isAiEmailAnalysisEnabled(source) {
  return String(source.AI_EMAIL_ANALYSIS_ENABLED || '').toLowerCase() === 'true';
}
...
function loadEnv(source = process.env) {
  ...
  // AI email analysis env is validated only when explicitly enabled.
  const aiEmailAnalysisEnabled = isAiEmailAnalysisEnabled(source);
  if (aiEmailAnalysisEnabled) {
    const provider = String(source.AI_PROVIDER || 'anthropic').toLowerCase();
    if (!AI_EMAIL_ANALYSIS_PROVIDERS.includes(provider)) {
      invalid.push('AI_PROVIDER');
    }
    if (provider === 'anthropic') {
      const hasKey = !isBlank(source.AI_API_KEY) || !isBlank(source.ANTHROPIC_API_KEY);
      if (!hasKey) {
  ...
  const result = envSchema.safeParse({
    ...
    AI_EMAIL_ANALYSIS_ENABLED: source.AI_EMAIL_ANALYSIS_ENABLED || undefined,
    AI_PROVIDER: source.AI_PROVIDER || undefined,
    AI_MODEL: source.AI_MODEL || undefined,
    AI_API_KEY: source.AI_API_KEY || undefined,
    STAGING_ADMIN_TOOLS: source.STAGING_ADMIN_TOOLS || undefined,
...
module.exports = {
  ...
  AI_EMAIL_ANALYSIS_PROVIDERS,
  hasGmailConfigSignal,
  isAiEmailAnalysisEnabled,
