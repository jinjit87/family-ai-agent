...
/**
 * Archive a payment (status ARCHIVED).
 * Intentionally preserves paidAt when the payment was previously paid
 * (historical payment time is kept, same pattern as Tasks completedAt).
 * Soft-deleted rows remain excluded separately.
 * @param {string} id
 * @returns {Promise<{ payment: object } | { notFound: true } | { conflict: string }>}
 */
async function archivePayment(id) {
  ...
  const payment = await prisma.payment.update({
    ...
    data: {
      status: 'ARCHIVED',
      // Do not touch paidAt — preserve historical payment time.
    },
  });
  return { payment: serializePayment(payment) };
}
...
/**
 * Totals grouped by businessUnit AND currency — never combine unlike currencies.
 * @param {PaymentRecord[]} payments
 * @returns {{ businessUnit: string, currency: string, total: string }[]}
 */
function sumTotalsByBusinessUnitAndCurrency(payments) {
  /** @type {Map<string, { businessUnit: string, currency: string, total: import('@prisma/client').Prisma.Decimal }>} */
  const map = new Map();
  for (const payment of payments) {
    const key = `${payment.businessUnit}\u0000${payment.currency}`;
    const current = map.get(key);
    if (current) {
      current.total = current.total.add(new Prisma.Decimal(payment.amount));
    } else {
      map.set(key, {
        businessUnit: payment.businessUnit,
        currency: payment.currency,
  ...
  return Array.from(map.values())
    ...
    .sort((a, b) => {
      const bu = a.businessUnit.localeCompare(b.businessUnit);
      return bu !== 0 ? bu : a.currency.localeCompare(b.currency);
    });
}
 ...
 *
 * Weekly window boundary rules (inclusive ends for due-soon):
 * - dueInNext7Days: now <= dueDate <= now+7days AND status !== PAID
 * - overdue: dueDate < now AND not PAID/CANCELLED/ARCHIVED (and not soft-deleted)
 * - due exactly at `now` → dueInNext7Days (not overdue)
 * - due exactly at `now+7days` → dueInNext7Days
 * - due after `now+7days` → neither bucket (unless already overdue, which it is not)
 *
 * @param {{ now?: Date }} [options]
 */
async function getWeeklyReport(options = {}) {
  ...
  const dueSoon = rows.filter(
    (p) =>
      ...
      p.dueDate.getTime() <= inSevenDays.getTime() &&
      p.status !== 'PAID'
  );
  ...
  const byBusinessUnit = sumTotalsByBusinessUnitAndCurrency(reportPayments);
