...
function createInboxRouter({ adminAuth }) {
  ...
  router.get('/important', async (req, res) => {
    ...
    try {
      const result = await inbox.listImportantInboxItems(parsed.data);
      return res.status(200).json(result);
    } catch (_err) {
      logInboxFailure('list important items');
      return res.status(500).json({ error: 'Failed to list important inbox items' });
    }
  });
  ...
  router.get('/tasks', async (req, res) => {
    ...
    try {
      const result = await inbox.listActionInboxItems(parsed.data);
      return res.status(200).json(result);
    } catch (_err) {
      logInboxFailure('list action items');
      return res.status(500).json({ error: 'Failed to list inbox action items' });
    }
  });
  ...
  router.get('/bills', async (req, res) => {
    ...
    try {
      const result = await inbox.listBillInboxItems(parsed.data);
      return res.status(200).json(result);
    } catch (_err) {
      logInboxFailure('list bill items');
      return res.status(500).json({ error: 'Failed to list inbox bills' });
    }
  });

  // Batch / selective analysis (must be before /:id)
  router.post('/analyze', async (req, res) => {
    const parsed = schemas.analyzeInboxBatchSchema.safeParse(req.body ?? {});
    if (!parsed.success) {
      return res.status(400).json(schemas.formatZodError(parsed.error));
    }

    try {
      const result = await inbox.analyzeInboxBatch(parsed.data);
      return res.status(200).json(result);
    } catch (_err) {
      logInboxFailure('batch analyze');
      return res.status(500).json({ error: 'Failed to analyze inbox messages' });
    }
  });
  ...
  router.post('/', async (req, res) => {
    ...
    try {
      ...
      if (result.inactiveAccount) {
        return res.status(409).json({ error: 'Inbox account is inactive' });
      }
      if (result.conflict) {
        return res.status(409).json({ error: result.conflict });
      }
  ...
  router.post('/:id/analyze', async (req, res) => {
    ...
    try {
      // Single-item path allows re-analysis of already-processed messages.
      const result = await inbox.analyzeInboxItem(parsed.data.id, { allowReanalyze: true });
      if (result.notFound) {
        return res.status(404).json({ error: 'Inbox item not found' });
      }
      if (result.busy) {
        return res.status(409).json({ error: 'Inbox item is already being analyzed', code: 'ANALYSIS_IN_PROGRESS' });
      }
      if (result.skipped) {
        return res.status(409).json({ error: `Cannot analyze item in status ${result.skipped}`, code: 'SKIPPED' });
      }
      if (result.failed) {
