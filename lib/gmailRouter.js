 ...
 * @param {string} action
 */
function logGmailFailure(action) {
  // Never log tokens, codes, secrets, or raw provider errors.
...
function createGmailRouter({ adminAuth, env }) {
  ...
  router.get('/callback', async (req, res) => {
    try {
      const result = await gmail.handleOAuthCallback(req.query, env);
      if (result.account) {
      ...
      logGmailFailure('complete oauth callback');
      if (req.query.format === 'json') {
  ...
  router.get('/connect', adminAuth, (req, res) => {
    try {
      const result = gmail.buildConnectUrl(env);
      ...
      logGmailFailure('start oauth connect');
  ...
  router.post('/sync-all', adminAuth, async (_req, res) => {
    try {
      ...
      logGmailFailure('sync all accounts');
  ...
  router.post('/accounts/:id/disconnect', adminAuth, async (req, res) => {
    ...
    try {
      ...
      logGmailFailure('disconnect account');
