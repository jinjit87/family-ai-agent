 ...
 */
async function persistAnalysisResult(id, analysis) {
  const prisma = db.getPrisma();
...
async function analyzeInboxItem(id, options = {}) {
  ...
  try {
    ...
    await persistAnalysisResult(id, analysis);
...
async function listImportantInboxItems(query) {
  return listInboxItemsWithWhere(query, {
...
async function getDailyBriefing(options = {}) {
  ...
  const [highPriority, actionItems, bills, packages, securityAlerts, overdue] = await Promise.all([
    prisma.inboxItem.findMany({
      where: {
        ...active,
        urgency: { in: ['HIGH', 'URGENT', 'CRITICAL'] },
